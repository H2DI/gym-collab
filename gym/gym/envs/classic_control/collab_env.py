...
class Agent(object):
    ...
    def __init__(self, x=0., y=0.5, length=0.5, height=0.5, agent_number=0):
        ...
        self.on_firm_ground = True # = can jump
    ...
    def up_down_update(self, agents, obstacles, jump=False): # handles gravity and up/down movement collisions
        ...
        if self.on_firm_ground:
            # print str(self.agent_number) + " is on firm ground"
            if jump:
                new_vy = min(self.vy + (self.jump_force / self.mass - self.gravity) * self.tau , self.max_vy)
        ...
        for agent in agents:
            if not(self.agent_number == agent.agent_number):
                if (new_y < agent.y + self.height + agent.height) and (self.y > agent.y) and (abs(self.x - agent.x) < self.length + agent.length):
                ...
                if (abs(self.y - agent.y) <= self.height + agent.height) and (abs(self.x - agent.x) < (self.length + agent.length)):
                    contact = True
        ...
        for obstacle in obstacles:
            if (new_y < obstacle.y + self.height + obstacle.height) and (self.y > obstacle.y) and (abs(self.x - obstacle.x) < self.length + obstacle.length):
    ...
    def left_right_update(self, agents, obstacles, direction=0):
        new_vx = min( max(self.vx + direction * self.force_mag / self.mass * self.tau, self.min_vx), self.max_vx)
